...
import {
  ...
  Activity,
  CheckCircle2,
  ShoppingBag, // Imported for the Bags icon
} from "lucide-react";
...
interface FormState {
  truckNumber: string;
  godownNumber: number | "";
  bags: number | ""; // Added bags to state
...
export default function KajliTruckEntryClient() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Logic to handle numeric fields (godownNumber and bags)
    const isNumericField = name === "godownNumber" || name === "bags";
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const payload: KajliTruckEntries = {
        ...
        bags: Number(formData.bags), // Add bags to payload
      ...
      <Card className="w-full max-w-md bg-zinc-900 border-zinc-800 shadow-xl">
        ...
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Grid for Godown, Cargo, and Bags */}
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Bags Input (New Field) - Spanning full width for emphasis or keep in grid */}
              <div className="space-y-2 col-span-2">
                <Label
                  htmlFor="bags"
                  className="text-zinc-300 flex items-center gap-2"
                >
                  <ShoppingBag className="w-4 h-4 text-zinc-500" /> Total Bags
                </Label>
                <Input
                  id="bags"
                  name="bags"
